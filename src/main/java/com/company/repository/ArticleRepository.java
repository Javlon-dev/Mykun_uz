...
public interface ArticleRepository extends JpaRepository<ArticleEntity, Integer> {
    ...
    Page<ArticleEntity> findByVisibleAndStatus(Boolean visible, Pageable pageable, ArticleStatus status);
    ...
    @Query(value = "select a.id,a.title,a.description,a.attach_id,a.published_date " +
            ...
            nativeQuery = true)
    List<ArticleSimpleMapper> getTypeId(@Param("typeId") Integer typeId, @Param("status") String status);
  ...
  /*  @Modifying
    ...
    @Transactional
    @Modifying
    @Query(value = "update ArticleEntity set viewCount = viewCount + 1 where id =:id")
